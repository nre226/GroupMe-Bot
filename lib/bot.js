...
class Bot {
    ...
    static getGroupId() {
        ...
        return "After apiRequest";

        // On error
        apiRequest.on('error', function(error) {
            return 'Error posting message ' + JSON.stringify(error);
        });

        // On timeout
        apiRequest.on('timeout', function(error) {
            return 'Timeout posting message ' + JSON.stringify(error);
        });
